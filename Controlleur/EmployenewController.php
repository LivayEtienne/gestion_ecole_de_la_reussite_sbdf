...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($employeModel->verifierEmailExistant($email)) {
        ...
        header("Location: ../View/Employe/employe-new/employe-new.php?error=1&message=Email déjà utilisé");
        exit();
    }
    ...
    if ($employeModel->verifierTelephoneExistant($telephone)) {
        // Rediriger avec un message d'erreur
        header("Location: ../View/Employe/employe-new/employe-new.php?error=1&message=Telephone déjà utilisé");
        exit();
    }
    ...
    header("Location: ../View/Employe/employe-new/employe-new.php?success=1&nom=$nom&prenom=$prenom&email=$email&role=$role");
    exit();
}
